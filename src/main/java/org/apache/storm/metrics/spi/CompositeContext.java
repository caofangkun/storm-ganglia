...
public class CompositeContext extends AbstractMetricsContext {
  ...
  private static final Logger LOG = LoggerFactory
      .getLogger(CompositeContext.class);
  ...
  public void init(String contextName, ContextFactory factory) {
    ...
    for (int i = 0; i < nKids; ++i) {
      MetricsContext ctxt =
          MetricsUtil.getContext(String.format(SUB_FMT, contextName, i),
              contextName);
      if (null != ctxt) {
        subctxt.add(ctxt);
      }
  ...
  public MetricsRecord newRecord(String recordName) {
    return (MetricsRecord) Proxy.newProxyInstance(MetricsRecord.class
        .getClassLoader(), new Class[] { MetricsRecord.class },
  ...
  protected void emitRecord(String contextName, String recordName,
    ...
    for (MetricsContext ctxt : subctxt) {
      try {
        ((AbstractMetricsContext) ctxt).emitRecord(contextName, recordName,
            outRec);
