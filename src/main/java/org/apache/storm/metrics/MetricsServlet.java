...
import org.apache.storm.metrics.spi.OutputRecord;
import org.apache.storm.metrics.spi.AbstractMetricsContext.MetricMap;
import org.apache.storm.metrics.spi.AbstractMetricsContext.TagMap;
import org.mortbay.util.ajax.JSON;
import org.mortbay.util.ajax.JSON.Output;

/**
 * A servlet to print out metrics data. By default, the servlet returns a
 * textual representation (no promises are made for parseability), and users can
...
public class MetricsServlet extends HttpServlet {
  ...
  /**
   * Collects all metric data, and returns a map: contextName -> recordName -> [
   * (tag->tagValue), (metric->metricValue) ]. The values are either String or
   * Number. The final value is implemented as a list of TagsMetricsPair.
   */
  Map<String, Map<String, List<TagsMetricsPair>>> makeMap(
    ...
    for (MetricsContext context : contexts) {
      ...
      for (Map.Entry<String, Collection<OutputRecord>> r : context
          .getAllRecords().entrySet()) {
  ...
  void printMap(PrintWriter out,
      Map<String, Map<String, List<TagsMetricsPair>>> map) {
    for (Map.Entry<String, Map<String, List<TagsMetricsPair>>> context : map
        .entrySet()) {
      out.println(context.getKey());
      for (Map.Entry<String, List<TagsMetricsPair>> record : context.getValue()
          .entrySet()) {
        indent(out, 1);
        ...
        for (TagsMetricsPair pair : record.getValue()) {
          ...
          for (Map.Entry<String, Number> metricValue : pair.metricMap
              .entrySet()) {
            indent(out, 3);
