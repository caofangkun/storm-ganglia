...
/**
 * The MetricsTimeVaryingLong class is for a metric that naturally varies over
 * time (e.g. number of files created). The metrics is accumulated over an
 * interval (set in the metrics config file); the metrics is published at the
 * end of each interval and then reset to zero. Hence the counter has the value
 * in the current interval.
 * 
 * Note if one wants a time associated with the metric then use
 * 
 * @see org.apache.storm.metrics.util.MetricsTimeVaryingRate
 * 
 */
public class MetricsTimeVaryingLong extends MetricsBase {

  private static final Logger LOG = LoggerFactory
      .getLogger("com.tencent.jstorm.metrics.util");
   ...
   * 
   * @param nam the name of the metrics to be used to publish the metric
   * @param registry - where the metrics object will be registered A description
   *          of {@link #NO_DESCRIPTION} is used
   */
  public MetricsTimeVaryingLong(final String nam, MetricsRegistry registry) {
