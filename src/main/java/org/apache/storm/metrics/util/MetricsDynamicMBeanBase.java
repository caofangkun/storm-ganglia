 ...
 * limitations under the License.
 */
package org.apache.storm.metrics.util;
 ...
 * from metrics. The metrics constructors registers metrics in a registry.
 * Different categories of metrics should be in differnt classes with their own
 * registry (as in NameNodeMetrics and DataNodeMetrics). Then the MBean can be
 * created passing the registry to the constructor. The MBean should be then
 * registered using a mbean name (example): MetricsHolder myMetrics = new
 * MetricsHolder(); // has metrics and registry MetricsTestMBean theMBean = new
 * MetricsTestMBean(myMetrics.mregistry); ObjectName mbeanName =
 * MBeanUtil.registerMBean("ServiceFoo", "TestStatistics", theMBean);
 * 
 * 
 */
public abstract class MetricsDynamicMBeanBase implements DynamicMBean {
  ...
  protected MetricsDynamicMBeanBase(final MetricsRegistry mr,
      final String aMBeanDescription) {
    metricsRegistry = mr;
  ...
  private void createMBeanInfo() {
    ...
    List<MBeanAttributeInfo> attributesInfo =
    ...
    for (MetricsBase o : metricsRegistry.getMetricsList()) {
      ...
      if (MetricsTimeVaryingRate.class.isInstance(o)) {
        ...
        attributesInfo.add(new MBeanAttributeInfo(o.getName() + AVG_TIME,
            "java.lang.Long", o.getDescription(), true, false, false));
        attributesInfo.add(new MBeanAttributeInfo(o.getName() + MIN_TIME,
            "java.lang.Long", o.getDescription(), true, false, false));
        attributesInfo.add(new MBeanAttributeInfo(o.getName() + MAX_TIME,
            "java.lang.Long", o.getDescription(), true, false, false));
        needsMinMaxResetOperation = true; // the min and max can be reset.

        // Note the special attributes (AVG_TIME, MIN_TIME, ..) are derived from
        // metrics
        ...
        attributesInfo.add(new MBeanAttributeInfo(o.getName(),
            "java.lang.Integer", o.getDescription(), true, false, false));
        ...
        MetricsUtil.LOG
            .error("unknown metrics type: " + o.getClass().getName());
      }

      if (needsMinMaxResetOperation) {
        operationsInfo =
            new MBeanOperationInfo[] { new MBeanOperationInfo(
                RESET_ALL_MIN_MAX_OP, "Reset (zero) All Min Max", null, "void",
    ...
    mbeanInfo =
        new MBeanInfo(this.getClass().getName(), mbeanDescription,
