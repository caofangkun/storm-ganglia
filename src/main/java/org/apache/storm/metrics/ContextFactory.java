...
import org.apache.storm.metrics.spi.NullContext;
...
public class ContextFactory {
	...
	private Logger logger = LoggerFactory.getLogger(ContextFactory.class);
	private static final String PROPERTIES_FILE = "storm-metrics-default.properties";
	private static final String USER_PROPERTIES_FILE = "storm-metrics.properties";
	private static final String CONTEXT_CLASS_SUFFIX = ".class";
	private static final String DEFAULT_CONTEXT_CLASSNAME = "com.tencent.jstorm.metrics.spi.NullContext";
	 ...
	 * instance of <code>com.tencent.jstorm.metrics.spi.NullContext</code>,
	 * which is a dummy "no-op" context which will cause all metric data to be
	 * discarded.
	 * 
	 * @param contextName
	...
	private void setAttributes() throws IOException {
		// String tmp = getClass().getResource("/").toString();
		// System.out.println(tmp);
		// InputStream is = getClass().getResourceAsStream(PROPERTIES_FILE);
		List<URL> resources = Utils.findResources(PROPERTIES_FILE);
		if (resources.size() > 0) {
			logger.info("Resource:" + resources.get(0).toString());
			InputStream is = resources.get(0).openStream();
			if (is != null) {
		...
		resources = Utils.findResources(USER_PROPERTIES_FILE);
		if (resources.size() > 0) {
			logger.info("Resource:" + resources.get(0).toString());
			InputStream is = resources.get(0).openStream();
			if (is != null) {
